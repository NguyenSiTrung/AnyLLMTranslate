...
- **Layout translation full-text access**: Replace hover-only expansion with click/keyboard activation. Render clipped layout boxes as focusable `role="button"` elements, show a persistent popover on activation, and dismiss it via `Escape`, outside click, or selecting another block. (from: pdf-translation-ux_20260615, archived 2026-06-15)
- **Clipping detection for layout boxes**: Exact DOM measurement is unavailable at render time, so estimate overflow from text length, box width, and computed font size. Flag boxes whose estimated rendered height exceeds the original box by a safety margin (e.g. >20%). (from: pdf-translation-ux_20260615, archived 2026-06-15)
