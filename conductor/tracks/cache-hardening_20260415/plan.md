...
- [x] Task 1: Write failing tests for cached/uncached split in `handleTranslate`
...
- [x] Task 2: Implement cache split + merge in `handleTranslate`
...
- [x] Task 3: Run tests + lint, commit
  - `pnpm test` — all passing
  - `pnpm lint` — clean
  - Commit: `feat(cache): integrate IndexedDB cache into page translation pipeline` (98aa581)
...
- [x] Task 1: Check and add `alarms` permission
  <!-- files: wxt.config.ts -->
  - Inspect `wxt.config.ts` manifest permissions array
  - Add `"alarms"` if not already present

- [x] Task 2: Write failing tests for eviction scheduling
...
- [x] Task 3: Implement eviction in `entrypoints/background.ts`
  <!-- files: entrypoints/background.ts -->
  <!-- depends: task2 -->
...
- [x] Task 4: Run tests + lint, commit
  <!-- files: entrypoints/background.ts, wxt.config.ts -->
  <!-- depends: task3 -->
  - `pnpm test` — all passing
  - `pnpm lint` — clean
  - Commit: `feat(cache): schedule daily eviction via chrome.alarms` (ef9c448)
...
- [x] Task 1: Write failing tests for batched LRU writes
...
- [x] Task 2: Implement debounced LRU flush in `getCachedTranslation`
...
- [x] Task 3: Run full suite + lint, commit
  - `pnpm test` — 396 tests passing (3 UI tests fail pre-existing due to missing @testing-library/dom dep, not introduced by this track)
  - `pnpm lint` — clean
  - Commit: `perf(cache): batch LRU lastAccessedAt writes to reduce IndexedDB overhead` (aad9cc4)
