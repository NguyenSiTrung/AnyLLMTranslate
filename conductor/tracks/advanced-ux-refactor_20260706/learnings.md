...
## [2026-07-06 15:27] - Phase 8: PDF Card Polish + Micro-Polish + Final Verification (FR-6, FR-12)
- **Implemented:** (1) Wrapped the conditional "Never auto-open" field in `animate-fade-in-up` + `aria-live="polite"` and added a "Will skip:" parsed-host chip preview. (2) Added range `hint`s to all 4 number inputs via the existing `Input` `hint` prop; replaced the orphan `<p>(unlimited)</p>` with an `<Badge variant="info">Unlimited</Badge>`. (3) Final verification. 2 new tests (45 total).
- **Files changed:** `entrypoints/options/sections/AdvancedSection.tsx`, `entrypoints/options/__tests__/AdvancedSection.test.tsx`.
- **Learnings:**
  - **Convention (animation class):** The plan said `animate-fade-in` but the project's established class (used by every section wrapper) is `animate-fade-in-up`. Used the project convention; always grep for an existing animation utility before inventing a class name.
  - **Pattern (range hints without primitive extension):** The `Input` component already exposes a `hint` prop (`<p className="text-xs text-zinc-500 mt-1">` shown only when `!error`). Passing `hint="1–365 days"` satisfies the "inline range hint" requirement without a trailing-unit right-slot (which would have required extending `Input.tsx` and growing the bundle/primitive surface). Units already live in the field labels; hints reinforce the range.
  - **Bundle-delta measurement (worktree method):** To measure `wxt build` output vs the pre-track baseline without disturbing the working tree: `git worktree add --detach /tmp/alt-prebuild <pre-track-sha>`, symlink `node_modules` (`ln -sf .../node_modules <wt>/node_modules`), `wxt build` inside the worktree, compare the `chunks/options-*.js` size, then `git worktree remove --force` + `git worktree prune`. No stash/checkout gymnastics in the main tree.
  - **Bundle result:** options chunk +4916 B raw / **+1073 B gzipped (1.05 kB)** vs pre-track `5182da4`. Marginally over the aspirational <1 kB-gzipped guard (by ~49 B). Attributed to genuine new UI (hero cache readout, variable-insert chips, pre-export callout, range hints, Customized/Unlimited badges, parsed-host preview, AdvancedDisclosure usage) + 2 shared primitives (`Textarea`, `useCacheStats`). No bloat — all new bytes are feature code or reusable primitives. Accepted as justified by scope (8 phases / 12 FRs); shaving ~50 gzipped bytes would require stripping features or harming readability.
  - **Pre-existing failures (NOT this track's regressions):** `SubtitlesSection.test.tsx` has 1 failing test ("Fetch ASS + DOM fallback" text not found) — confirmed pre-existing by stashing Phase 8 changes and re-running (fails identically). `eslint .` reports 4 errors in `content/subtitleRenderer.ts`, `inject/jsonParseSubtitleHook.ts`, `services/__tests__/background.test.ts` — all untouched by this track. Filed a bd follow-up issue for the SubtitlesSection test. This track's files are tsc/test/lint clean.
---
