...
## [2026-07-07 18:20] - Phase 7 + Phase 8: Settings UI + Hardening
- **Implemented:** AdvancedSection "Translation Quality" card (5 toggles + 3 number fields in AdvancedDisclosure); ProviderKeyRow per-key concurrency/interval in AdvancedDisclosure; 5 new UI tests; lint fixes (non-null assertions, unused vars).
- **Files changed:** entrypoints/options/sections/AdvancedSection.tsx, entrypoints/options/components/ProviderKeyRow.tsx, entrypoints/options/__tests__/AdvancedSection.test.tsx, lib/richTranslate.ts, content/translationDisplay.ts, entrypoints/content.ts, services/__tests__/background.test.ts, services/__tests__/background.webStream.test.ts, services/__tests__/providerPool.test.ts
- **Commits:** 2eb9004, ede7f20
- **Learnings:**
  - **AdvancedDisclosure content is conditionally rendered (not CSS-hidden):** Tests that query inputs inside an AdvancedDisclosure must first click the disclosure button to expand it before `getByLabelText` can find the input. Pattern: `fireEvent.click(screen.getByRole('button', { name: /disclosure label/i }))` then query.
  - **Toggle label via `getByLabelText`:** The Toggle component with `label` prop creates a label association, so `screen.getByLabelText('Toggle label text')` finds the toggle input. `fireEvent.click` on it triggers `onChange`.
  - **Stagger index shift:** Adding a new card to AdvancedSection requires incrementing all subsequent `stagger(N)` indices by 1 to maintain the entrance animation order. Existing tests that don't assert on stagger indices are unaffected.
  - **Lint neutrality target:** Track code should introduce 0 new lint errors. Pre-existing errors (subtitleRenderer.ts, jsonParseSubtitleHook.ts) are excluded. Fix all new non-null assertions (`!`) with if-guards, unused vars with `_` prefix or removal, and `let` → `const` where never reassigned.
  - **Final quality gate results:** 1487 tests passing (135 files), 0 failures. 84 new tests total (target ≥30). tsc: only 8 pre-existing subtitlePrompt.test.ts errors. Lint: only 2 pre-existing errors. Build: 3.94 MB (delta ~20KB from 3.92MB baseline, well within +30KB gzipped budget).
