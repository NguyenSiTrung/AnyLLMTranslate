...
5. **Seek-Aware Priority Queue:** If the user jumps forward or backward in the video (`seeked` event), the content script must notify the background worker to prioritize translating the chunk at the new timestamp, moving it to the front of the queue.
6. **Overlay Defaulting:** Because native video players (like YouTube/Udemy) do not reliably support streaming updates to a WebVTT track, the `subtitleCoordinator` will immediately hand over intercepted cues to the Custom Subtitle Overlay (`subtitleOverlay.ts`), which is designed for dynamic cue updates.
...
- [ ] Skipping ahead in the timeline triggers a Priority Queue update, resulting in translated subtitles appearing seamlessly at the new location without waiting for linear completion.
