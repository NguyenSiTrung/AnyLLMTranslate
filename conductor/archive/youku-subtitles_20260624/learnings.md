...
## [2026-06-24] Phase 4.1: XHR endpoint discovery spike — NO ENDPOINT FOUND
- **Method:** DevTools network capture during live playback of `vid=1620234666` ("Tàng Hải Truyện 01", youku.tv watch page, DRM/FairPlay-CBCS, VIP content).
- **Finding: There is no separate subtitle XHR/fetch endpoint.** Youku delivers subtitles as **WebVTT segments embedded inside the HLS (CMAF) m3u8 manifest**, which the KUI player downloads and renders as SVG `<text>` into `#subtitle`. Decoded from the `ykplayer_process` telemetry beacon's `gokey` param:
  - `m3u8URL = http://pl-ali.youku.tv/playlist/m3u8?vid=<base64id>&type=cmfv4hd3`
  - `media_type=standard,subtitle` ← subtitle tracks bundled in the manifest, not a separate API
  - `drm_type_value=cbcs` ← Widevine CBCS encryption
  - Available variants (`mcmfa_info`, decoded): `vi` (Vietnamese, cmfa1sd), `guoyu` (Mandarin, cmfa1hd), `en` (English, cmfa1sd), `th` (Thai, cmfa1sd).
- **Telemetry 499 errors are noise:** `gm.mmstat.com/yt/*`, `yt.mmstat.com/*`, `yt1.mmstat.com/*`, and `youku.pcweb.control` are Alibaba "goldlog" analytics beacons (aplus SDK). HTTP 499 = nginx "Client Closed Request" (page navigated before beacon finished). They fail with or without our extension (ad-blockers, etc.) and are unrelated to subtitle translation.
- **Conclusion: Phase 4 (XHR interception) is correctly skipped.** DOM cue scraping is confirmed as the right approach — there is no higher-fidelity API path short of intercepting + decrypting HLS m3u8 subtitle variants, which is disproportionate to the fidelity gain over the working DOM scraper. This documents *why* Phase 4 was skipped so a future session doesn't re-investigate.
- **If higher-fidelity cues are ever needed:** parse the HLS master playlist to resolve the subtitle variant URL, fetch + concat `.vtt` segments, and handle CBCS-encrypted segments. Requires HLS parser + likely a Widevine session. Not worth it for subtitle translation timing.
