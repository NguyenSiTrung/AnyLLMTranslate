...
- [x] In `entrypoints/content.ts` line 114, replace hardcoded `setPageState('dual')` with:
...
- [x] Write test in `entrypoints/__tests__/content.test.ts`:
...
- [x] In `entrypoints/content.ts` `initInteractionFeatures()`, add to the existing
...
- [x] In `content/translationDisplay.ts`, update function signature to:
      `export function togglePageState(displayMode?: DisplayMode): PageState`
- [x] Add import: `import type { DisplayMode } from '@/types/config';`
- [x] Change the `'off'` branch: `next = displayMode === 'translation-only' ? 'translation-only' : 'dual';`
- [x] Update `content/__tests__/translationDisplay.test.ts`:
...
- [x] In `entrypoints/content.ts` `toggleTranslation()`, load settings and pass
...
- [x] If `togglePageState()` is called directly anywhere else, audit and update those call-sites.
...
- [x] In `entrypoints/options/__tests__/AdvancedSection.test.tsx`, find all
      `displayMode: 'dual'` occurrences and replace with `displayMode: 'bilingual-below'`
- [x] Run full test suite: `pnpm test` — confirm 459+ tests pass, no regressions
- [x] Run lint: `pnpm lint` — confirm clean
- [x] Commit: `fix(content): wire displayMode setting to page state and keyboard toggle`
