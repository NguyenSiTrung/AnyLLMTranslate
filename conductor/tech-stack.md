<!-- conductor-refresh: 2026-09-10 all (package/lock still unchanged; no new CI workflows this window; 592 pass / 0 fail across 208 files; tsc 0 / lint 0 re-run (no-invalid-void-type regression at subtitleCoordinator.test.ts:3399 found + fixed this session); build 3.82 MB re-measured; new pure/source modules: spaNavigationWatcher, chunkStability, pdfPageSelection, DeepLearning.AI adapter + handler; 73 archived / 0 active; Beads 2 open + 2 in progress) -->
...
- Quality gates snapshot (2026-09-10): **592** Vitest TCs across **208** test files; the full default `vitest run` completed **592 pass / 0 failures** (wall-clock 467 s — cold run; the 2026-08-11 baseline was 37.5 s). `tsc --noEmit` **0 errors** and **eslint 0 errors** (both re-run 2026-09-10). The refresh caught an eslint regression at `content/__tests__/subtitleCoordinator.test.ts:3399` (`Promise.withResolvers<void>()` → `@typescript-eslint/no-invalid-void-type`, introduced by the ASR Stop-cancellation work) and it was fixed in-session via `Promise.withResolvers<undefined>()`; production build **3.82 MB** (re-measured 2026-09-10 vs 3.77 MB). No `package.json` or `pnpm-lock.yaml` dependency changes were detected since the previous refresh (deps unchanged since 2026-08-05). CI/CD is unchanged this window — `pages.yml` (GitHub Pages guide deploy) and `bridge-image.yml` (GHCR scientific-pdf-bridge image) remain the only two workflows.
- Source added since 2026-08-11 (no new npm deps): `content/spaNavigationWatcher.ts` (SPA nav + absolute metadata URL matching), `content/chunkStability.ts`, `lib/pdfPageSelection.ts` (pure page-range math + merged continued translations), `content/playerChrome/adapters/deepLearningAi.ts` + `inject/subtitleHandlers/deepLearningAi.ts` + `inject/deepLearningAi.ts` (DeepLearning.AI platform). Python bridge gained `tests/test_pages_selection.py` and dropped `tests/test_smoke.py` (gitignored — AV quarantine loop).
- Tooling/ops: the `.agents` skill library was re-synced (`51536f6`) and a project `LICENSE` was added; `.gitignore` gained the Python smoke-test entry. Neither changes the extension build.
