<!-- conductor-refresh: 2026-09-12 all (package/lock still unchanged — deps frozen since 2026-08-05, 6th consecutive window; no new CI workflows; 754 pass / 0 fail across 214 files in 42 s; coverage 78.81 % stmts; tsc 0 / lint 0 re-run; build 3.85 MB; round-2 subtitle hardening added shared worldHandlers registry + MSE delta accumulation + MAX_MANIFEST_CUES bounds + transient-only retry — no new deps; stale audit §7 refs corrected to §8; 73 archived / 0 active; Beads 0 open / 0 in progress — 279 closed) -->
...
- Quality gates snapshot (2026-09-12): **754** Vitest TCs across **214** test files; the full default `vitest run` completed **754 pass / 0 failures** in 42 s. `tsc --noEmit` **0 errors** and **eslint 0 errors** (both re-run 2026-09-12 — no regression this window). Coverage re-measured: **78.81 % statements / 75.50 % branches / 82.37 % functions**. Production build **3.85 MB** (WXT total; `du` 3.7 MB). `package.json` and `pnpm-lock.yaml` are unchanged since 2026-08-05 (6th consecutive window with no dependency drift). CI/CD unchanged — `pages.yml` (GitHub Pages guide deploy) and `bridge-image.yml` (GHCR scientific-pdf-bridge image) remain the only two workflows.
