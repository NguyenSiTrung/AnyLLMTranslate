...
describe('translateParagraphs kind propagation', () => {
  it('tags math paragraphs with kind: "math" and keeps them verbatim', async () => {
    vi.mocked(chrome.runtime.sendMessage).mockImplementation(async (message: unknown) => {
      const action = (message as { action: string }).action;
      if (action === 'CLASSIFY_PDF_PARAGRAPHS') {
  ...
  it('tags figure-labeled paragraphs with kind: "figure" and keeps them verbatim', async () => {
    vi.mocked(chrome.runtime.sendMessage).mockImplementation(async (message: unknown) => {
      const action = (message as { action: string }).action;
      if (action === 'CLASSIFY_PDF_PARAGRAPHS') {
        return { success: true, labels: { '1-0': 'figure', '1-1': 'prose' } };
  ...
  it('tags translated prose paragraphs with kind: "prose"', async () => {
    vi.mocked(chrome.runtime.sendMessage).mockImplementation(async (message: unknown) => {
      const action = (message as { action: string }).action;
      if (action === 'CLASSIFY_PDF_PARAGRAPHS') {
        const pieces = (message as { paragraphs: Array<{ id: string }> }).paragraphs;
    ...
    const results = await translateParagraphs(
      [
        { pageNumber: 1, paragraph: { id: '1-0', text: 'Normal prose sentence.', fontSize: 12, isHeading: false, x: 0, y: 0, width: 0, height: 0 } },
      ],
      'https://example.com/a.pdf',
    );
