...
import type { AvailableSubtitleTrack, SubtitleTracksDiscoveredPayload, SubtitleTextTrackCuesPayload } from '@/types/subtitle';
import { findPrimaryVideo } from '@/lib/findPrimaryVideo';
import { extractTrackCues } from '@/lib/textTrackCues';

/** Track which TextTracks we've already emitted full cues for (avoid double-emit) */
const emittedTextTracks = new WeakSet<TextTrack>();

/** Scan a video element's textTracks and send discovery message + full cues */
function scanVideoTracks(video: HTMLVideoElement, bridge: MessageBridgeSender): void {
  ...
  for (let i = 0; i < video.textTracks.length; i++) {
    ...
    if (track.kind === 'subtitles' || track.kind === 'captions') {
      ...
      // Emit full cues if track.cues is populated and we haven't emitted yet
      if (track.cues && track.cues.length > 0 && !emittedTextTracks.has(track)) {
        const cues = extractTrackCues(track);
        if (cues.length > 0) {
          emittedTextTracks.add(track);
          bridge.send('SUBTITLE_TEXTTRACK_CUES', {
            cues,
            language: track.language || '',
            label: track.label || track.language || '',
            url: domTrack?.src || undefined,
          } as SubtitleTextTrackCuesPayload);

          console.log('AnyLLMTranslate: TextTrack full cues emitted', {
            language: track.language,
