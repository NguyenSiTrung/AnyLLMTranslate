...
describe('richTranslate', () => {
  describe('INLINE_ELEMENTS / isInlineTagName', () => {
    it('whitelists core inline tags case-insensitively', () => {
  ...
  describe('encodeInlineHtml', () => {
    it('leaves plain/empty/void content alone and encodes inline markup with attrs', () => {
      ...
      // Block tags stay verbatim; only inline children encode.
  ...
  describe('decodeInlineHtml', () => {
    it('round-trips placeholders, simulates LLM text swap, and handles multiples', () => {
      ...
      expect(frag.querySelector('strong')?.textContent).toBe('world');

      const link = encodeInlineHtml('Click <a href="https://example.com">here</a>');
      expect(
        decodeInlineHtml(link.flatText, link.variables).querySelector('a')?.getAttribute('href'),
      ).toBe('https://example.com');

      const nested = encodeInlineHtml('<a href="/x">bold <strong>link</strong></a>');
      const nestedFrag = decodeInlineHtml(nested.flatText, nested.variables);
      expect(nestedFrag.querySelector('a strong')?.textContent).toBe('link');
