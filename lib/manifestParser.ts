...
import type { AvailableSubtitleTrack, SubtitleSegmentFetchTemplate } from '@/types/subtitle';
...
export function parseDashManifest(body: string, baseUrl: string): DashSubtitleTrack[] {
  ...
  return extractSubtitleTracks(doc, baseUrl).map((track) => {
    const result: DashSubtitleTrack = {
      url: track.url,
      language: track.language,
    };
    if (track.segmentUrls) result.segmentUrls = track.segmentUrls;
    if (track.segmentFetch) result.segmentFetch = track.segmentFetch;
    return result;
  });
}
...
export function detectManifestTracks(
  ...
  if (isDash) {
    ...
    return tracks.map((t) => {
      const result: AvailableSubtitleTrack = {
        language: t.language,
        label: t.language,
        url: t.url,
        isAutoGenerated: false,
      ...
      if (t.segmentUrls) result.segmentUrls = t.segmentUrls;
      if (t.segmentFetch) result.segmentFetch = t.segmentFetch;
      return result;
    });
  }
