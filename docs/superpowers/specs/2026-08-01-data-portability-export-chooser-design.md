...
- **Passphrase dialog is missing standard affordances.** No show/hide toggle
  despite the "if you forget the passphrase, the backup is unrecoverable"
  stakes, and no strength feedback beyond "min 8 characters".
...
4. Passphrase dialog gains a **show/hide eye toggle** (export + import modes)
   and a **live strength hint** (export mode only).
...
- **Show/hide toggle:** eye icon button (lucide `Eye`/`EyeOff`) inside each
  passphrase field, toggling input `type` between `password` and `text`.
  Present in both export (`requireConfirm`) and import modes. Rendered as an
  absolutely positioned button in a `relative` wrapper (the shared `Input`
  has no suffix slot); `aria-label="Show passphrase"` / `"Hide passphrase"`,
  `tabIndex={-1}` so it does not break the field-to-field Tab order.
...
- New: eye toggle switches input `type` between `password` and `text`.
...
- Eye toggle is `type="button"` with `aria-label` and `aria-pressed`.
