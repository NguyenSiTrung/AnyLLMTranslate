...
interface CoordinatorState {
  ...
  /** Automatic YouTube activation attempts already made in this navigation. */
  youtubeAutoActivationKeys: Set<string>;
...
interface YoutubeTrackIdentity {
  videoId: string;
  language: string;
  kind: string;
}
...
/**
 * Identity of a YouTube timedtext URL for track matching: same video, same
 * caption language (translation target wins over source), same kind. Prefix
 * comparison was too loose — `url.split('&')[0]` is `…?v=ID`, which matches
 * every track of the video and could report a human/translated track as ASR.
 */
function youtubeTrackIdentityOf(url: string): YoutubeTrackIdentity | null {
  try {
    const u = new URL(url, window.location.href);
    return {
      videoId: u.searchParams.get('v') ?? '',
      language: u.searchParams.get('tlang') ?? u.searchParams.get('lang') ?? '',
...
function findYoutubeTrackMeta(url: string): AvailableSubtitleTrack | undefined {
  const target = youtubeTrackIdentityOf(url);
  if (!target) return undefined;
  return state.availableTracks.find((t) => {
    if (t.platform !== 'youtube' || !t.url) return false;
    const candidate = youtubeTrackIdentityOf(t.url);
    return (
      !!candidate &&
      candidate.videoId === target.videoId &&
...
/** True when the body is already JSON3 (the canonical word-level form). */
function isJsonBody(body: string): boolean {
  return body.trimStart().startsWith('{');
}

/** YouTube timedtext host/path guard for the canonical json3 refetch. */
function isYoutubeTimedtextUrl(url: string): boolean {
  try {
    const u = new URL(url, window.location.href);
    return /(^|\.)youtube\.com$/i.test(u.hostname) && u.pathname.startsWith('/api/timedtext');
  } catch {
    return false;
  }
}

interface AsrRealignRun {
  cancelled: boolean;
  removeProgressListener: () => void;
}

/**
 * In-flight YouTube ASR AI re-align runs. Overlapping pipelines are possible
 * (proactive timedtext fetch plus a passive intercept), so every run is tracked:
 * the mini-progress Stop button (via cancelBackgroundSubtitleSession) must
 * cancel all of them, not just the most recently started one.
 */
const activeAsrRealigns = new Set<AsrRealignRun>();

/**
 * Cancel every in-flight AI re-align: mark it cancelled, detach its progress
 * listener, and clear the progress/toast UI. The pipeline then bails out before
 * translating. No-op when no re-align is running.
 */
function cancelActiveAsrRealign(): void {
  if (activeAsrRealigns.size === 0) return;
  const runs = [...activeAsrRealigns];
  activeAsrRealigns.clear();
...
async function applyYoutubeAsrPipeline(options: {
  ...
  const aiEligible =
    asrEnable && asrAiEnable && platform === 'youtube' && isAsrTrack && rawCues.length > 0;

  // The player picks the intercepted fmt (often srv1/srv3), while the Settings
  // pre-align flow and the proactive fetch always use fmt=json3. Refetch the
  // canonical json3 body before building units so word-level timing and the
  // contentHash (=> cache key) match across flows. Fail-open to the intercepted
  // body when the refetch fails.
  let canonicalBody = body;
  if (aiEligible && !isJsonBody(body) && isYoutubeTimedtextUrl(url)) {
    try {
      const json3Body = await fetchSubtitleContent(buildJson3TimedtextUrl(url));
      if (parseYoutubeWords(json3Body).length > 0) canonicalBody = json3Body;
    } catch {
      // Keep the intercepted body: coarse units, possible cache miss.
    }
  }
  ...
  const realignRun: AsrRealignRun = {
    cancelled: false,
  ...
  activeAsrRealigns.add(realignRun);

  try {
    const units = prepareYoutubeAsrAiInput({ body: canonicalBody, cues: rawCues });
    if (units.length === 0) {
      return { cues, resegmentMode, asrEnable, asrAiEnable, cancelled: false };
    }
    ...
    activeAsrRealigns.delete(realignRun);
...
async function fetchSubtitleContent(
  url: string,
  canFetch: (() => boolean) | undefined = undefined,
): Promise<string> {
  if (canFetch && !canFetch()) {
    throw new Error('Subtitle fetch is no longer current');
  }
  try {
    ...
    return fetchViaBackground(url, canFetch);
  }
}
...
function requestYoutubeCaptionFallback(
  track: AvailableSubtitleTrack,
  canRequest: (() => boolean) | undefined = undefined,
): void {
  if (
    (canRequest && !canRequest()) ||
    !isCurrentYoutubeTimedtextTrack(track) ||
    !track.url ||
    state.youtubeCaptionFallbackUrls.has(track.url)
  ) return;
...
async function processTracksDiscovered(payload: SubtitleTracksDiscoveredPayload): Promise<void> {
  ...
  // YouTube may return player metadata for previews or stale players while a
  // watch page is open. Never let a known different video enter the track list.
  if (payload.platform === 'youtube' && incomingVideoId) {
    const currentVideoId = extractYoutubeVideoIdFromUrl(window.location.href);
    if (currentVideoId && incomingVideoId !== currentVideoId) {
      console.log('AnyLLMTranslate: Ignoring stale YouTube subtitle tracks', {
        currentVideoId,
        incomingVideoId,
      });
      return;
...
async function tryAutoActivate(epochAtStart: number): Promise<{ activated: boolean; reason: string }> {
  ...
  if (settings.subtitleSettings?.enabled && autoActivate && preferredLang) {
    ...
    if (preferred?.url) {
      ...
      await selectSubtitleTrack(preferred.language, { automatic: true });
...
async function tryYoutubeProactiveTimedtext(
  ...
  await selectSubtitleTrack(track.language, { automatic: true });
 ...
 */
function isCurrentYoutubeTimedtextTrack(track: AvailableSubtitleTrack): boolean {
  if (track.platform !== 'youtube' || !track.url || !track.videoId) return false;

  const currentVideoId = extractYoutubeVideoIdFromUrl(window.location.href);
  let timedtextVideoId: string | undefined;
  try {
    timedtextVideoId = extractYoutubeVideoIdFromUrl(
  ...
  return !!currentVideoId && currentVideoId === track.videoId && currentVideoId === timedtextVideoId;
}

type SelectSubtitleTrackOptions = {
  automatic?: boolean;
};

export async function selectSubtitleTrack(
  language: string,
  options: SelectSubtitleTrackOptions = {},
): Promise<void> {
  ...
  if (track.platform === 'youtube') {
    if (!isCurrentYoutubeTimedtextTrack(track)) {
      console.log('AnyLLMTranslate: Skipping YouTube timedtext request for a stale track');
      return;
    }
    if (options.automatic) {
      const key = `${track.videoId}:${track.language}:${track.isAutoGenerated ? 'asr' : 'manual'}`;
      if (state.youtubeAutoActivationKeys.has(key)) {
        console.log('AnyLLMTranslate: Skipping duplicate YouTube auto-activation');
        return;
      }
      state.youtubeAutoActivationKeys.add(key);
    }
  }
...
/** Coalesce concurrent proactive YouTube timedtext activations per navigation and URL. */
...
async function activateYoutubeTrackViaPipeline(track: AvailableSubtitleTrack): Promise<void> {
  if (!track.url) return;
  if (!isCurrentYoutubeTimedtextTrack(track)) {
    console.log('AnyLLMTranslate: Skipping YouTube timedtext fetch for a stale track');
    return;
  }
  const pipelineKey = `${state.navigationEpoch}:${track.url}`;
  if (youtubePipelineInflight.has(pipelineKey)) return;
  if (state.fetchedTrackUrls.has(track.url) && (state.isOverlayMode || state.activeSubtitleSessionId !== null)) {
    return;
  }

  youtubePipelineInflight.add(pipelineKey);
  try {
    await activateYoutubeTrackViaPipelineInner(track);
  } finally {
    youtubePipelineInflight.delete(pipelineKey);
  }
}
...
async function activateYoutubeTrackViaPipelineInner(track: AvailableSubtitleTrack): Promise<void> {
  ...
  const isCurrentActivation = () =>
    state.navigationEpoch === epochAtStart && isCurrentYoutubeTimedtextTrack(track);
  ...
  try {
    ...
    body = await fetchSubtitleContent(buildJson3TimedtextUrl(track.url), isCurrentActivation);
  } catch (error) {
    if (!isCurrentActivation()) return;
    console.error('AnyLLMTranslate: Failed to fetch YouTube timedtext', error);
    requestYoutubeCaptionFallback(track, isCurrentActivation);
    return;
  }
  if (!isCurrentActivation()) return;
