...
import { applyYoutubeAsrResegment } from '@/lib/youtubeAsrResegment';
...
async function handleIntercepted(payload: SubtitleInterceptedPayload, requestId: string): Promise<void> {
  ...
  try {
    ...
    const asrEnable = settings.subtitleSettings.youtubeAsrResegment?.enable ?? true;
    const trackMeta = state.availableTracks.find(
      (t) =>
        t.platform === 'youtube' &&
        !!t.url &&
        (t.url === url || url.startsWith(t.url) || t.url.startsWith(url.split('&')[0])),
    );
    const cues = applyYoutubeAsrResegment({
      platform,
      url,
      body,
      cues: rawCues,
    ...
    if (cues !== rawCues || cues.length !== rawCues.length) {
