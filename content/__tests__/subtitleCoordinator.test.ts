...
describe('subtitleCoordinator – YouTube ASR first-load pipeline', () => {
  ...
  it('never fetches a YouTube track that does not belong to the current video', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: false,
      status: 404,
    ...
    // Phase A: the discovered track belongs to another video entirely.
    await _capturedTracksHandler!({
      platform: 'youtube',
      videoId: 'other-video',
      tracks: [{
        language: 'en',
        label: 'English (auto-generated)',
        url: 'https://www.youtube.com/api/timedtext?v=other-video&lang=en&kind=asr',
        isAutoGenerated: true,
        platform: 'youtube',
        videoId: 'other-video',
      }],
    ...
    const mod = await import('@/content/subtitleCoordinator');
    expect(mod.getAvailableTracks()).toEqual([]);
    await mod.selectSubtitleTrack('en');
    expectNoFetch();

    // Phase B: a track discovered for the current video goes stale after
    // navigation to a different video.
    mod.resetCoordinatorState();
    fetchMock.mockClear();
    sendMessageMock.mockClear();
    vi.mocked(mockInjectSendMessage).mockClear();
...
describe('subtitleCoordinator – DOM auto-activation and toast scoping', () => {
  ...
  it('tryAutoActivateForDom manual: toasts when overlay is missing, succeeds when visible', async () => {
    ...
    const mod = await import('@/content/subtitleCoordinator');
    const missing = await mod.tryAutoActivateForDom({ manual: true });

    expect(missing.activated).toBe(false);
    expect(mockShowSubtitleToast).toHaveBeenCalledWith(
    ...
    const visible = await mod.tryAutoActivateForDom({ manual: true });
    expect(visible.activated).toBe(true);
    expect(mockShowSubtitleToast).not.toHaveBeenCalled();
  });

  it('tryAutoActivateForDom shows platform-specific toasts (Max, Youku) when captions are off', async () => {
    const platformHandler = (platform: string, selector: string) => ({
      platform,
      detect: vi.fn(() => true),
      isWatchPage: vi.fn(() => true),
      getPatterns: vi.fn(() => []),
      ...
      getDomCueSource: vi.fn(() => ({
        ...
        captionWindowSelector: selector,
        observeRootSelector: 'body',
        readActiveLanguage: () => '',
        captionHideMethod: 'visibility' as const,
      })),
    ...
    mockDetectCurrentHandler.mockReturnValue(
      platformHandler('hbomax', '[data-testid="cueBoxRowTextCue"]'),
    );
    const maxResult = await mod.tryAutoActivateForDom();
    expect(maxResult.activated).toBe(false);
    expect(mockShowSubtitleToast).toHaveBeenCalledWith(
      'Enable subtitles in Max to enable translation (Alt+S to retry).',
    );

    mockShowSubtitleToast.mockClear();
    mockDetectCurrentHandler.mockReturnValue(platformHandler('youku', '#subtitle'));
    const youkuResult = await mod.tryAutoActivateForDom();
    expect(youkuResult.activated).toBe(false);
    expect(mockShowSubtitleToast).toHaveBeenCalledWith(
...
describe('subtitleCoordinator – MSE SourceBuffer delta tier', () => {
  ...
  it('activates on the first payload, merges deltas, and dedupes re-appended segments', async () => {
    ...
    // First SourceBuffer segment activates the overlay and is translated.
    ...
    // Second segment merges in; only the delta is translated.
    mockUpdateCues.mockClear();
    await _capturedMseCuesHandler({
      cues: [{ startTime: 3, endTime: 4, text: 'b' }],
      platform: 'mse',
    ...
    // and no third translation request may be issued — only a re-push.
    mockUpdateCues.mockClear();
    await _capturedMseCuesHandler({
      cues: [{ startTime: 1, endTime: 2, text: 'a' }],
      platform: 'mse',
...
describe('subtitleCoordinator – untranslated section notice', () => {
  ...
  it('tells the user once when every retry of a manifest delta fails', async () => {
    ...
    // A second failing delta does not repeat the notice.
